...
# Fig 3
at_least_one_error_plot_file <- "plots/at_least_one_error_post.pdf"
# Fig 4
...
# Fig 6
difficulty_barplot_by_model_file <- "plots/difficulty_barplot_by_model.pdf"
# Fig 7
misclassified_barplot_file <- "plots/misclassified_barplot.pdf"

# Tab 2
list_of_models_table <- "tex/list_of_models.tex"
...
# Order models by post avg_Pass
models_order <-  model_data[which(model_data$cond == "post"),"Short name"][order(model_data[which(model_data$cond == "post"),"median_Pass"], decreasing = T)]
...
# consider only pre and only post tables
task_data_post <- task_data[which(task_data$cond == "post"),]
task_data_pre <- task_data[which(task_data$cond == "pre"),]
model_data_post <- model_data[which(model_data$cond == "post"),]

# order models in replica data
replica_data$Model <- factor(replica_data$`Short name`, levels = models_order)


########################
# Compute unsolved TB, i.e., TB with thask not solved (n_valid == 0)
...
########################
# Figure 2
# Boxplot mean Pass and accpet@1 : Post and Pre
...
pass_boxplot <- ggplot(task_data, aes(x = Model, y = avg_p_Pass, fill = `Open model`)) +
  ...
  ylab("Mean Pass Tests per Task") + theme(plot.margin=grid::unit(c(0,0,0,0), "mm"))
pass_boxplot
ggsave(filename = pass_boxplot_file, plot = pass_boxplot, device = "pdf", width = 3.5, height = 3.8, units = "in", dpi = 300 )


########################
# Figure 3
# Count statically detectable errors: Post

at_least_one_error_plot <- ggplot(task_data_post, aes(x=Model, y=n_at_least_one_error, fill=`Open model`)) +
  geom_boxplot(outliers = T, colour="gray60", linewidth = 0.2, outlier.color = "gray30", outlier.size = 0.8, outlier.alpha = 0.5, outlier.shape = 16) +
  scale_fill_manual(values = licence_col) +
  theme_se() +
  ...
  scale_y_continuous(breaks=c(0,2,4,6,8,10), labels=c(0,2,4,6,8,10),limits = c(0,10) )   + theme(plot.margin=grid::unit(c(0,0,0,0), "mm"))

at_least_one_error_plot
ggsave(filename = at_least_one_error_plot_file, plot = at_least_one_error_plot, device = "pdf", width = 3.5, height = 2.1, units = "in", dpi = 300 )
...
fp_post__boxplot <- ggplot(task_data_post, aes(x=Model, y=avg_p_FP, fill = `Open model`)) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1), axis.title.x = element_blank()) +
  theme(legend.position = "top", legend.box.spacing = unit(-5, units = "pt")  ) +
  ylab("False Positive") + theme(plot.margin=grid::unit(c(0,0,0,0), "mm"))
fp_post__boxplot 
...
fn_post__boxplot <- ggplot(task_data_post, aes(x=Model, y=avg_p_FN, fill = `Open model`)) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1), axis.title.x = element_blank()) +
  theme(legend.position = "top", legend.box.spacing = unit(-5, units = "pt")  ) +
  ylab("False Negative") + theme(plot.margin=grid::unit(c(0,0,0,0), "mm"))
fn_post__boxplot 
...
ggsave(filename = fp_and_fn_post_boxplot_file, plot = fp_and_fn_post_boxplot, device = "pdf", width = 3.5, height = 3.2, units = "in", dpi = 300 )
...
complexity_pattern_plot <- ggplot(data = unsolved_tb, aes(x=Task, y=n_unsolved )) +
  geom_bar_pattern(aes( pattern_angle =  cc, fill = Difficulty ), stat = "identity",
                   pattern_density = 0.1, 
                   pattern_colour = "gray80", pattern_size = 0.2, pattern_spacing =0.05,
                   pattern = "stripe"
  ) +
...
ggsave(filename = complexity_pattern_plot_file, plot = complexity_pattern_plot, device = "pdf", width = 3.5, height = 1.8, units = "in", dpi = 300 )


########################
# Figure 6
# barplot number of solved tasks for post

task_solved_post <- task_data_post[which(task_data_post$n_valid >0),]
#fwrite(task_solved,file = solved_tb_file, sep = "\t",row.names = F, col.names = T)

difficulty_barplot_by_model <- ggplot(task_solved_post, aes(x=Model, fill=Difficulty)) +
  geom_bar(stat='count') +
  scale_fill_manual(values = complexity_colors) +
  theme_se() +
  ylab("Number of post tasks with accept@1 > 0") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1), axis.title.x = element_blank()) +
  theme(legend.position = "top", legend.box.spacing = unit(-1, units = "pt")  )  + theme(plot.margin=grid::unit(c(0,0,0,0), "mm"))
difficulty_barplot_by_model
ggsave(filename = difficulty_barplot_by_model_file, plot = difficulty_barplot_by_model, device = "pdf", width = 3.5, height = 2.25, units = "in", dpi = 300 )
...
misclassified_barplot <- ggplot(model_data_post, aes(x=Model, y=tot_Misclassified/tot_ManualValid, fill = `Open model`)) +
  geom_bar(stat = 'identity') +
  geom_text(aes(label=tot_ManualValid), vjust=-0.3, size=1.8, colour = "gray20") +
  scale_fill_manual(values = licence_col) +
  theme_se() +
  scale_y_continuous(labels = percent, breaks=c(0,0.1,0.2,0.3)) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1), axis.title.x = element_blank()) +
  theme(legend.position = "top", legend.box.spacing = unit(-2, units = "pt")  ) +
  theme(panel.grid.major.x =  element_line(linewidth=0.1, colour = "gray90")) +
  ylab("Misclassified Solutions") +
  coord_cartesian(clip = "off") + theme(plot.margin=grid::unit(c(0,0,0,0), "mm"))
...
model_data_post$perc_mis <- model_data_post$tot_Misclassified/model_data_post$tot_ManualValid

########################
# Table 2
# List of LLMs

models_description[,"Open model"] <- licence_names[models_description$Licence]
models_description <- models_description[order(models_description$`Open model`),]
colnames(models_description)
models_description_save_cols <- c("Full name","Short name","Parameters (B)", "Size (GB)")
models_description_to_save <- models_description[,models_description_save_cols]


models_description_to_save_tex <- xtable(models_description_to_save, digits = c(2))
print(models_description_to_save_tex,
      file=list_of_models_table,
      append=F,table.placement = "htb",
      caption.placement="bottom", 
      hline.after=c(-1,0,nrow(models_description_to_save_tex)),
...
#   theme(legend.position = "top", legend.box.spacing = unit(-5, units = "pt")  ) +
#   theme(panel.grid.major.x =  element_line(linewidth=0.1, colour = "gray90")) +
#   ylab("Misclassified Solutions (Post)")
# misclassified_barplot
# ggsave(filename = misclassified_barplot_file, plot = misclassified_barplot, device = "pdf", width = 3.5, height = 1.9, units = "in", dpi = 300 )
# 
# 
